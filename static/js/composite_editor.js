...
/**
 * Добавление текстового слоя (class="tournament-name")
 */
...
function renderLayers() {
    ...
    if (layers.length === 0) {
        list.innerHTML = `
            <p class="text-muted small text-center py-3">
                Нет слоёв. Нажмите «iframe» или «Текст» для создания.
            </p>
        `;
    } else {
        list.innerHTML = layers.map((layer, idx) => {
            ...
            const icon     = isText ? 'fas fa-font' : 'fas fa-code';
            const label    = isText ? `Текст ${idx + 1}` : `iframe ${idx + 1}`;

            // Общие поля позиционирования
            const posFields = `
            ...
            // Специфичные поля
            const specificFields = isText ? `
                <div class="mb-2">
                    <label class="form-label small mb-0">Текст</label>
                    <textarea class="form-control form-control-sm" rows="2"
                              onchange="updateLayerProp(${idx}, 'text', this.value)">${escHtml(layer.text || '')}</textarea>
                </div>
                <div class="mb-1">
                    <label class="form-label small mb-0">Размер шрифта: <b>${layer.fontSize || 75}px</b></label>
                    <input type="range" class="form-range" min="20" max="200" step="1" value="${layer.fontSize || 75}"
                           oninput="updateLayerProp(${idx}, 'fontSize', Number(this.value))">
                </div>
            ` : `
                <div class="mb-2">
                    <label class="form-label small mb-0">URL</label>
                    <div class="input-group input-group-sm">
                        <input type="text" class="form-control" value="${escHtml(layer.url || '')}"
                ...
                <div class="mb-1">
                    <label class="form-label small mb-0">Масштаб: ${((layer.scale || 0.5) * 100).toFixed(0)}%</label>
                    <input type="range" class="form-range" min="0.1" max="2" step="0.05" value="${layer.scale || 0.5}"
                           oninput="updateLayerProp(${idx}, 'scale', Number(this.value))">
                </div>
            `;

            return `
                <div class="card layer-card mb-2 ${isActive ? 'selected' : ''}" data-index="${idx}">
                    <div class="card-header d-flex align-items-center" onclick="selectLayer(${idx})">
                        <i class="${icon} layer-handle me-2"></i>
    ...
    preview.innerHTML = layers.map((layer, idx) => {
        ...
        const innerHtml = isText
            ? `<div class="preview-text-layer tournament-name"
                    style="font-size:${layer.fontSize || 75}px; width:100%; height:100%;"
               >${escHtml(layer.text || '')}</div>`
            : (layer.url
                ? `<iframe src="${layer.url}" allowtransparency="true"
                           style="width:${(layer.width / (layer.scale || 0.5))}px;
                                  height:${(layer.height / (layer.scale || 0.5))}px;
                                  transform:scale(${layer.scale || 0.5});"></iframe>`
                : `<div style="width:100%;height:100%;background:rgba(100,100,100,0.3);
                              display:flex;align-items:center;justify-content:center;color:#fff;">
                       <i class="fas fa-plus fa-2x"></i>
                   </div>`);

        return `
            <div class="preview-layer ${isActive ? 'selected' : ''}"
                 data-index="${idx}"
                 style="left:${layer.x}px; top:${layer.y}px; width:${layer.width}px; height:${layer.height}px;"
                 onmousedown="startDrag(event, ${idx})">
                <div class="layer-label">${isText ? '⬜ Текст' : '🖼 iframe'} ${idx + 1}</div>
                ${innerHtml}
            </div>
        `;
    }).join('');
